...
def validate parallelstore_metrics(): 
#"Fetch and validate Parallelstore IOPS and throughput using Cloud Monitoring API
	...
	instance_id = CONFIG["parallelstore"]["instance name"] 
...
def fetch metric(metric_type, instance_id): 
...
#Query last 15 minutes of data 
	...
	Interval = monitoring v3.TimeInterval( 
		end_time={"seconds"; int(time.time())}. 
		start_time={"seconds": Int(time.time() - 1800)} # Last 30 minutes 
	
	)	

	filter_str = f'metric.type="{metric type}" AND resource.type = "parallelstore.googleapis.com/Instance" AND resource.label.instance_id="{instance_id}"' 
	  request = monitoring v3.ListTimeSeriesRequest( 
		name = project_name, 
		filter = filter_str. 
		interval = interval, 
	...
	lops_metric = "parallelstore.googleapis.com/instance/read_ops_count" 
...
logger.info(f"Max IOPS: {actual_lops} (Expected: {EXPECTED_JOPS})") 
...
assert actual_iops is not None and actual_iops > EXPECTED_IOPS, f"IOPS too low:{actual ops}" 
assert actual_throughput is not None and actual_throughput> EXPECTED_THROUGHPUT_MBPS, f"Throughput too low: {actual throughput} MBps" 
