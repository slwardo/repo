...
# --- Execution:
# --- ./diagnose_scrape.sh netd-tkjlg kube-system 10.64.0.2 10231
# --- ./diagnose_scrape.sh gke-metrics-agent-zkg6c kube-system 10.64.0.2 8202
# --- Configuration ---
    ...
    print_warning "Skipping listening checks."
      ...
      for CONTAINER_NAME in $CONTAINER_NAMES; do
          ...
          if [ $? -ne 0 ]; then
               ...
               if echo "$LOGS" | grep -Eq "(localhost|127\.0\.0\.1):$PORT\b"; then
           ...
           for CONTAINER_NAME in $CONTAINER_NAMES; do
               ...
               if command -v ss >/dev/null 2>&1; then
                    ...
                    if [ $? -eq 0 ]; then
                         ...
                         print_warning "'ss' command failed in container '$CONTAINER_NAME'."
                    fi
               else
                    print_info "'ss' command not found in container '$CONTAINER_NAME'. Trying 'netstat'..."
                    ...
                    if command -v netstat >/dev/null 2>&1; then
                         ...
                         print_warning "Neither 'ss' nor 'netstat' found in container '$CONTAINER_NAME'."
                    fi
               fi
...
if $IP_MISMATCH; then
    ...
    FINAL_CAUSE="IP Mismatch"
      ...
      echo "  -> NetworkPolicies (Step 7) if any exist."
      ...
      echo "* INFO: Analysis inconclusive. Could not confirm IP mismatch or localhost binding via automated checks."
       ...
       echo "  -> Check NetworkPolicies (Step 7) and GCP Firewalls."
